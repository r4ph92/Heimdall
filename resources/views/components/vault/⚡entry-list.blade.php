...
<div class="flex h-full animate-fadein">
    ...
    <div class="w-80 shrink-0 border-r border-gray-800 flex flex-col">
        <div class="px-4 py-4 border-b border-gray-800 space-y-3">
            <h2 class="text-sm font-semibold text-gray-300 uppercase tracking-wider">Vault</h2>
            <div class="relative">
                ...
                <input
                    ...
                    type="text"
                    placeholder="Search…"
                    class="w-full bg-gray-800 border border-gray-700 text-white text-sm rounded-lg pl-9 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
                >
            </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto py-2 space-y-0.5 px-2">
            @forelse ($this->entries as $entry)
                ...
                >
                    <div class="w-9 h-9 rounded-lg bg-indigo-600/30 flex items-center justify-center text-indigo-300 font-bold text-sm shrink-0">
                        {{ strtoupper(substr($entry->service_name, 0, 1)) }}
                    </div>
                    <div class="min-w-0">
                        <p class="text-sm font-medium text-white truncate">{{ $entry->service_name }}</p>
            ...
            >
                <div class="flex items-center justify-between mb-6">
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 rounded-xl bg-indigo-600/30 flex items-center justify-center text-indigo-300 font-bold text-lg">
                            {{ strtoupper(substr($decrypted['service_name'], 0, 1)) }}
                        </div>
                        <div>
                            <h2 class="text-xl font-semibold text-white">{{ $decrypted['service_name'] }}</h2>
                            <p class="text-xs text-gray-500">Updated {{ $decrypted['updated_at'] }}</p>
                        </div>
                    </div>
                    <button
                        wire:click="deleteEntry({{ $decrypted['id'] }})"
                        wire:confirm="Delete this entry? This cannot be undone."
                        class="text-gray-600 hover:text-red-400 transition-colors duration-150"
                    >
                ...
                <div class="space-y-3">
                    @if ($decrypted['username'])
                        ...
                        <div
                            class="bg-gray-900 border border-gray-800 rounded-xl p-4 flex items-center justify-between gap-3"
                            x-data="{ copied: false }"
                            data-copy="{{ $decrypted['username'] }}"
                        >
                            <div class="min-w-0">
                                <p class="text-xs text-gray-500 uppercase tracking-wider mb-1">Username</p>
                                <p class="text-white text-sm truncate">{{ $decrypted['username'] }}</p>
                            </div>
                            <button
                    ...
                    >
                        ...
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <div class="flex items-center gap-3 shrink-0">
                                <button @click="show = !show" class="text-xs text-gray-400 hover:text-white transition" x-text="show ? 'Hide' : 'Show'"></button>
                                <button
                                    @click="navigator.clipboard.writeText($el.closest('[data-copy]').dataset.copy).then(() => { copied = true; setTimeout(() => copied = false, 1500) })"
                    ...
                    @if ($decrypted['notes'])
                        <div class="bg-gray-900 border border-gray-800 rounded-xl p-4">
                            ...
                            <p class="text-gray-300 text-sm whitespace-pre-wrap">{{ $decrypted['notes'] }}</p>
                        </div>
                    @endif
                </div>
            ...
            <div class="h-full flex items-center justify-center animate-fadein">
                <div class="text-center">
                    <div class="w-16 h-16 rounded-2xl bg-gray-800 flex items-center justify-center mx-auto mb-4">
                        <svg class="w-8 h-8 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                    </div>
                    <p class="text-gray-500 text-sm">Select an entry to view details</p>
                    <p class="text-gray-600 text-xs mt-1">or create a new one</p>
                </div>
            </div>
        @endif
    </div>
